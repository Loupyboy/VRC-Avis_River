    ...
    void applyMatcap(inout float4 finalColor, inout float3 matcapEmission)
    {
        ...
        , _BlackLightMaskMatcap
        ...
        #ifdef COLOR_GRADING_HDR_3D
            half2 matcapUV2 = half2(dot(worldViewRight, poiMesh.normals[float(1)]), dot(worldViewUp, poiMesh.normals[float(1)])) * float(0.5) + 0.5;
            #if defined(PROP_MATCAP2) || !defined(OPTIMIZER_ENABLED)
                matcap2 = UNITY_SAMPLE_TEX2D_SAMPLER(_Matcap2, _MainTex, TRANSFORM_TEX(matcapUV2, _Matcap2)) * float4(0.6562665,0.6562665,0.6562665,1);
            #else
                matcap2 = float4(0.6562665,0.6562665,0.6562665,1);
            #endif
            matcap2.rgb *= float(1);
            #if defined(PROP_MATCAP2MASK) || !defined(OPTIMIZER_ENABLED)
            ...
            blendMatcap(finalColor, float(0), float(0), float(1), matcap2, matcap2Mask, matcapEmission, float(1)
            #ifdef POI_LIGHTING
            , float(0)
            #endif
            #ifdef POI_BLACKLIGHT
