...
void calculateBasePassLightMaps()
{
        ...
        switch(float(1))
        {
            ...
            {
                poiLight.rampedLightMap = saturate(1 - smoothstep(float(0) - .000001, float(0.328), 1 - poiLight.lightMap));
        ...
        if (float(0) == 2) // Wrapped
        {
            ...
            uint normalsindex = float(0) > 0 ? 1: 0;
                    ...
                    {
                        poiLight.rampedLightMap = lerp(float4(1,1,1,1) * lerp(poiLight.indirectLighting, 1, float(0)), float3(1, 1, 1), saturate(1 - smoothstep(float(0) - .000001, float(0.328), 1 - poiLight.lightMap)));
        ...
        float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
        {
            ...
            else //if(float(1) == 2) // Wrapped
            {
                ...
                poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep(float(0) - .000001, float(0.328), 1 - uv)));
