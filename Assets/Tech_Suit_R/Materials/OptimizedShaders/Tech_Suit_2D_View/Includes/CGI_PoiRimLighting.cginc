...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.461)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.461)) + lerp(albedo.rgb, rimColor, float(0.461)) * rimBrighten, rim);
}
#endif
